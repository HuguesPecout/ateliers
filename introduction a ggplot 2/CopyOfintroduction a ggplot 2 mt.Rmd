...
Le dataset palmerpenguins contiennt des mesures de tailles pour 3 espèces de pingouin observées sur 3 îles de l'[archipel Palmer en Antarxtique](https://fr.wikipedia.org/wiki/Archipel_Palmer)
...
- rajouter un thème, un titre, un sous titre et des libellés
...
```{r geom_jitter, exercise=TRUE, exercise.lines = 10}

```

```{r geom_jitter-solution}
ggplot(data = penguins) +
  geom_jitter(aes(x = species, y = body_mass_g, color = species), alpha = 0.5, show.legend = FALSE, position = position_jitter(width = 0.2, seed = 0)) +
  theme_dark() +
  labs(x = "Body mass (g)",
